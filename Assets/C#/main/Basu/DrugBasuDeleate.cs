...
public class DragItemA2D : MonoBehaviour
{
    private Vector3 dragOffset;
    private float zCoordinate;

private Vector3 originalPosition; // 元の位置を記憶
    private bool itemBDestroyed = false; // ItemBが消去されたかどうかのフラグ
...
// マウスボタンが離されたとき (ドラッグ終了/ドロップ時)
    void OnMouseUp()
    {
        ...
        // (デバッグ用) アイテムBが消えたかどうかは、ログや他のロジックで確認できます
        if (itemBDestroyed)
        {
            Debug.Log("【成功】ドロップ操作中にアイテムBは消去されました。");
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("basu"))
        {
            Debug.Log("basuと衝突検知: " + other.gameObject.name);
            
            // ItemBにアタッチされている消去スクリプトの関数を呼び出す
            ItemBBehavior itemBScript = other.gameObject.GetComponent<ItemBBehavior>();
            
            if (itemBScript != null)
            {
                itemBScript.RemoveItem();
                itemBDestroyed = true;
            }
        }
    }
